-- =============================================================================
-- 0010_location_aliases.sql
--
-- Purpose
--   Store alternate, former, local, translated, and title-style names for
--   canonical locations in the world-state schema. This table exists so the
--   system can resolve multiple surface names back to a single location entity,
--   optionally scoped to a historical validity window.
--
-- Row semantics
--   One row represents one alias fact for one canonical location, optionally
...
--   - Natural keys / uniqueness: `(location_id, alias_name)` is unique so the
--     same canonical location cannot carry the same alias text twice.
...
--
-- Relationships
--   - Owns FKs to `locations(location_id)` and optionally to
--     `eras(era_id)` through `start_era_id` and `end_era_id`.
--   - Downstream name-resolution, search, and ingestion logic should join this
--     table to `locations` on `location_id` to map observed alias strings back
--     to the canonical location entity.
--
-- Audit & provenance
...
--
-- Performance
--   A secondary index on `alias_name` supports alias-to-location lookup during
...
CREATE TABLE IF NOT EXISTS location_aliases (
    ...
    -- Surrogate primary key for this location alias.
    location_alias_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    CONSTRAINT location_aliases_uq_location_alias UNIQUE (
        location_id, alias_name
    ),

    CONSTRAINT location_aliases_chk_alias_name_nonempty
    CHECK (length(btrim(alias_name)) > 0),

    CONSTRAINT location_aliases_chk_alias_type
    CHECK (
        alias_type IN (
            'alternate_name',
            'former_name',
    ...
    CONSTRAINT location_aliases_chk_year_nonnegative
    CHECK (
        (start_year IS NULL OR start_year >= 0)
        AND (end_year IS NULL OR end_year >= 0)
    ),

    CONSTRAINT location_aliases_chk_notes_nonempty
...
CREATE INDEX IF NOT EXISTS idx_location_aliases_alias_name
ON location_aliases (alias_name);
...
'One row per alternate or former location name, optionally time-bounded.';

COMMENT ON COLUMN location_aliases.location_alias_id IS
'Primary key for this location alias row (UUID).';

COMMENT ON COLUMN location_aliases.location_id IS
'Owning location.';

COMMENT ON COLUMN location_aliases.alias_name IS
'Alias text for the location.';

COMMENT ON COLUMN location_aliases.alias_type IS
'Alias category (former_name/local_name/etc.).';

COMMENT ON COLUMN location_aliases.start_era_id IS
'Optional start era for alias validity.';

COMMENT ON COLUMN location_aliases.start_year IS
'Optional start year-in-era for alias validity.';

COMMENT ON COLUMN location_aliases.end_era_id IS
'Optional end era for alias validity.';

COMMENT ON COLUMN location_aliases.end_year IS
'Optional end year-in-era for alias validity.';

COMMENT ON COLUMN location_aliases.notes IS
'Optional notes for this alias record.';

COMMENT ON COLUMN location_aliases.created_at IS
'UTC timestamp recording row creation.';

COMMENT ON INDEX idx_location_aliases_alias_name IS
'Index to accelerate alias-based location lookup.';
